...
bool loadOBJ( const char * path,
    ...
    std::vector< GLfloat > output_uvs;
    std::vector< GLuint > output_indices;
    ...
    for( unsigned int i=0; i<vertexIndices.size(); i++ ){
        ...
        unsigned int indiceIndex = vertexIndices[i];
        ...
        out_indices.push_back( indiceIndex );
...
// Writing some processVertex() function or something?
...
//            unsigned int vertexIndex[3], uvIndex[3], normalIndex[3];
            ...
            if ( matches != 9 ){
                printf( "Bad 'f' segment in obj file: %s\n", path.c_str() );
                fclose( file );
                return false;
            }
        }
